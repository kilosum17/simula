...
import { getAnyConf, getPetConf } from "shared/help/pet_catalog"
    ...
    return (
        <LBox Size={size} Pos={pos} AutoSize="Y" Wraps HAlign="Center" Trans SortOrder="LayoutOrder"
            ...
            {items.map((item, i) => {
                ...
                return (
                    <GroupItemCell key={item.id} Size={size} Aspect={1} item={item} showSelect={showSelect}
...
function GroupItemCell({ item, showSelect, counts, setCount, Size, Aspect, LayoutOrder, MinSize }: {
    ...
    const conf = getAnyConf(item.id)!
    const damage = tostring(conf.damage)
    const body = (
        <NTooltip data={{ kind: 'ITEM', itemId: item.id }} disable={showSelect} >
            <LBox Size={new UDim2(1, 0, 1, 0)} NoList Trans  >
                <LImage Size={new UDim2(1, 0, 1, 0)} Image={conf.icon!} />
                <LText Pos={new UDim2(1, 0, 1, 0)} Size={new UDim2(1, 0, 0.3, 0)}
                    AnchorPoint={new Vector2(1, 1)} Text={damage} Align="Right"
                    StrokeThickness={2} Color={col('white')} Visible={conf.isPet} />
                <LText Pos={new UDim2(1, 0, 0, 0)} Size={new UDim2(1, 0, 0.2, 0)}
