import { useEffect } from "@rbxts/react"
import { RunService } from "@rbxts/services"
import { getStageBoardPart } from "shared/help/assist"
import { getStageCostV2 } from "shared/help/DATA"
import { formatNumber } from "shared/help/helpers"
import { getProgStageNo, getRebirth } from "shared/signals/player_attributes"
import { usePlayerData } from "shared/ui/hooks/use_player_data"

export const StageBoardOne = ({ stageNo }: {
    stageNo: number
}) => {
    const board = getStageBoardPart(stageNo)
    const { balance, rebirth, progStage } = usePlayerData()
    const amount = formatNumber(getStageCostV2(stageNo, rebirth))

    const getCost = () => {
        return getStageCostV2(stageNo, rebirth)
    }

    const MouseClicked = () => {
        // miner.buyStage(nextStage)
    }

    useEffect(() => {
        const open = stageNo > getProgStageNo()
        board.CanCollide = open
        if (RunService.IsStudio()) board.CanCollide = false
        board.CanQuery = false
        board.CanTouch = false
        board.Transparency = open ? 0.1 : 1
    })

    const enabled = (() => {
        const open = stageNo > getProgStageNo()
        board.CanCollide = open
        if (RunService.IsStudio()) board.CanCollide = false
        board.CanQuery = false
        board.CanTouch = false
        board.Transparency = open ? 0.1 : 1
        return open
    })()
    let face = 'Left' as 'Left' | 'Right'
    if (nextStage > 35) {
        face = 'Right'
    }
    const showRebirth = miner.client.storeMap().map(s => {
        return shouldRebirth(nextStage, s.stats.rebirth)[0]
    })
    const rebirthNo = miner.client.storeMap().map(s => {
        const no = shouldRebirth(nextStage, s.stats.rebirth)[1]
        return `Rebirth ${no}`
    })
    const showBuy = showRebirth.map(s => !s)
    return (
        <surfacegui MaxDistance= { 800} Face = { face } Enabled = { enabled } >
            {/* BUY UI */ }
            < LBox Size = { new UDim2(1, 0, 1, 0) } Vert Center Trans SpaceX = { new UDim(0.2, 0) } Visible = { showBuy }
    AnchorPoint = { new Vector2(0.5, 0.5) } MaxSize = { new Vector2(600, 400) } Pos = { new UDim2(0.5, 0, 0.5, 0) } >
        <LPusher gapS={ 0.05 } />
            < LText Text = {`Area ${nextStage}`
} Size = { new UDim2(1, 0, 0.25, 0) } Var = "white" StrokeThickness = { 2} />
    <LPusher gapS={ 0.05} />
        < CostUI size = { new UDim2(1, 0, 0.3, 0) } syncher = { miner.client.syncher }
getCost = { getCost } short
    />
    <LPusher gapS={ 0.05 } />
{/* <LButton Text="Buy Now" MouseClicked={MouseClicked} /> */ }
<OldLButton Text="Buy Now" onClick = { MouseClicked } Size = { new UDim2(0.8, 0, 0.3, 0) } />
    </LBox>
{/* REBIRTH UI */ }
<LBox Size={ new UDim2(1, 0, 1, 0) } Trans SpaceX = { new UDim(0.2, 0) } Visible = { showRebirth } NoList
AnchorPoint = { new Vector2(0.5, 0.5) } MaxSize = { new Vector2(600, 400) } Pos = { new UDim2(0.5, 0, 0.5, 0) } >
    <LImage Image={ icons.Tel_Rebirth } Size = { new UDim2(1, 0, 0.8, 0) } Aspect />
        <LBox Trans Pos = { new UDim2(0.5, 0, 0.5, 0) } AnchorPoint = { new Vector2(0.5, 0) } Size = { new UDim2(0.6, 0, 0.4, 0) } Vert HAlign = "Center" >
            <LText Text={ rebirthNo } Size = { new UDim2(0.8, 0, 0.4, 0) } Var = "white" StrokeThickness = { 3} />
                <LPusher gapS={ 0.01 } />
                    < LText Text = {`Tons of Rewards!`} Size = { new UDim2(1, 0, 0.3, 0) } Var = "yellow" StrokeThickness = { 3} />
                        <LPusher gapS={ 0.01 } />
                            < NButton Text = { 'Rebirth!'} Var = "green" Size = { new UDim2(0.6, 0, 0.2, 0) }
onClick = { MouseClicked } corner = { 4} Small MinSize = { new Vector2(70, 25) } />
    </LBox>
    </LBox>
    </surfacegui>
    )
}
