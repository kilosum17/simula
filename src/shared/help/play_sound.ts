...
import { chooseRandom, getHRP, getIgnoreFold } from "./assist"
...
export const playSoundType = ({ ver, verName, name, pos, volume = 0.5, looped = false }: {
    ver: 'NAMED' | 'TYPED', verName?: TVerName,
    ...
    if (ver === 'TYPED') {
        ...
        soundId = chooseRandom(soundMap[verName!])
    } else {
        soundId = sounds[name!]
        if (!soundId) {
